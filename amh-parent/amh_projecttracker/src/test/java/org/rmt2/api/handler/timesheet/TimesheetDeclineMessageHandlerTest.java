...
/**
 * Test the timesheet decline process of the Project Tracker Api.
 * 
 * @author rterrell
 * 
 */
...
public class TimesheetDeclineMessageHandlerTest extends BaseProjectTrackerMessageHandlerTest {
    private static final int TIMESHEET_ID = 111;
    public static final String API_ERROR = "Test Error: Timesheet API decline error occurred";
    ...
    private void createInputData() {
        ...
        this.timesheetExt.setStatusId(TimesheetConst.STATUS_DECLINED);
        this.timesheetExt.setStatusName("Declined");
    }

    @Test
    public void testSuccess_Approve() {
        String request = RMT2File.getFileContentsAsString("xml/timesheet/TimesheetDeclineRequest.xml");
        
        try {
            when(this.mockApi.decline(isA(Integer.class))).thenReturn(TimesheetConst.STATUS_DECLINED);
        } catch (Exception e) {
            Assert.fail("Failed to setup stub for decline timesheet method");
        }
        ...
        try {
            results = handler.processMessage(ApiTransactionCodes.PROJTRACK_TIMESHEET_DECLINE, request);
        ...
        String msg = RMT2String.replace(TimesheetMessageHandlerConst.MESSAGE_POST_SUBMIT_SUCCESS, "declined", "%s");
        ...
        for (TimesheetType ts : actualRepsonse.getProfile().getTimesheet()) {
            ...
            Assert.assertEquals("Declined", ts.getStatus().getName());
    ...
    public void testError_API_Error() {
        ...
        try {
            when(this.mockApi.decline(isA(Integer.class))).thenThrow(new TimesheetApiException(API_ERROR));
        } catch (Exception e) {
            Assert.fail("Failed to setup stub for decline timesheet method");
        }
        ...
        String msg = RMT2String.replace(TimesheetMessageHandlerConst.MESSAGE_POST_SUBMIT_ERROR, "declining", "%s");
        Assert.assertEquals(msg, actualRepsonse.getReplyStatus().getMessage());
    ...
    public void testSuccess_Timesheet_Not_Exists() {
        ...
        try {
            when(this.mockApi.decline(isA(Integer.class))).thenReturn(0);
        } catch (Exception e) {
            Assert.fail("Failed to setup stub for decline timesheet method");
        }
    ...
    public void testValidation_Missing_Profile() {
        String request = RMT2File.getFileContentsAsString("xml/timesheet/TimesheetDeclineMissingProfileRequest.xml");
    ...
    public void testValidation_Missing_Timesheet() {
        String request = RMT2File.getFileContentsAsString("xml/timesheet/TimesheetDeclineMissingTimesheetRequest.xml");
    ...
    public void testValidation_Too_Many_Timesheets() {
        String request = RMT2File.getFileContentsAsString("xml/timesheet/TimesheetDeclineTooManyTimesheetsRequest.xml");

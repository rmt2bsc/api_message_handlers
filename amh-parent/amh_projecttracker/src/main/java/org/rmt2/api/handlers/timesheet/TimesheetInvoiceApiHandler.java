...
/**
 * Handles and routes Timesheet Invoicing related messages to the Project
 * Tracker Administration API.
 * 
 * @author roy.terrell
 *
 */
public class TimesheetInvoiceApiHandler extends
        AbstractJaxbMessageHandler<ProjectProfileRequest, ProjectProfileResponse, List<TimesheetType>> {
    
    private static final Logger logger = Logger.getLogger(TimesheetInvoiceApiHandler.class);
    protected ObjectFactory jaxbObjFactory;
    protected InvoiceTimesheetApi api;

    /**
     * Default constructor for TimesheetInvoiceApiHandler
     * 
     * @param payload
     */
    public TimesheetInvoiceApiHandler() {
        super();
        this.api = InvoiceTimesheetApiFactory.createApi(ProjectTrackerApiConst.APP_NAME);
        this.jaxbObjFactory = new ObjectFactory();
        this.responseObj = jaxbObjFactory.createProjectProfileResponse();
        logger.info(TimesheetInvoiceApiHandler.class.getName() + " was instantiated successfully");
    }
     ...
     * @return
     */
    protected List<TimesheetType> buildJaxbInvoiceResults(TimesheetDto dto) {
        List<TimesheetType> list = new ArrayList<>();
        TimesheetType jaxbObj = TimesheetTypeBuilder.Builder.create()
                .withTimesheetId(dto.getTimesheetId())
                .build();

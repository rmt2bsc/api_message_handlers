...
/**
 * Common Role message handler which works with the Authentication/Security API.
 * 
 * @author roy.terrell
 *
 */
public abstract class RoleApiHandler extends
        AbstractJaxbMessageHandler<AuthenticationRequest, AuthenticationResponse, List<RoleType>> {
    
    private static final Logger logger = Logger.getLogger(RoleApiHandler.class);
    protected ObjectFactory jaxbObjFactory;
    protected List<RoleType> jaxbObj;
    protected MessageHandlerCommonReplyStatus rs;

    /**
     * @param payload
     */
    public RoleApiHandler() {
        super();
        this.jaxbObjFactory = new ObjectFactory();
        this.responseObj = jaxbObjFactory.createAuthenticationResponse();
        this.jaxbObj = null;
        logger.info(RoleApiHandler.class.getName() + " was instantiated successfully");
    }
    ...
    @Override
    protected String buildResponse(List<RoleType> payload, MessageHandlerCommonReplyStatus replyStatus) {
        ...
        if (payload != null) {
            ...
            this.responseObj.getProfile().getRoleInfo().addAll(payload);
        }

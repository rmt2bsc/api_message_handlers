...
import org.rmt2.api.handlers.admin.user.UserDeleteApiHandler;
...
public class UserDeleteMessageHandlerTest extends BaseAuthenticationMessageHandlerTest {
     ...
     * 
     */
    public UserDeleteMessageHandlerTest() {
        return;
    }
    ...
    public void testSuccess_Delete() {
        String request = RMT2File.getFileContentsAsString("xml/admin/user/UserDeleteRequest.xml");
        try {
            when(this.mockApi.deleteUser(isA(Integer.class))).thenReturn(1);
        } catch (UserApiException e) {
            Assert.fail("Unable to setup mock stub for delete user record");
        }
        MessageHandlerResults results = null;
        UserDeleteApiHandler handler = new UserDeleteApiHandler();
        try {
            results = handler.processMessage(ApiTransactionCodes.AUTH_USER_DELETE, request);
        ...
        Assert.assertEquals(UserMessageHandlerConst.MESSAGE_DELETE_SUCCESS, actualRepsonse.getReplyStatus()
                .getMessage());
    }
    ...
    public void testSuccess_NotFound() {
        String request = RMT2File.getFileContentsAsString("xml/admin/user/UserDeleteRequest.xml");
        try {
            when(this.mockApi.deleteUser(isA(Integer.class))).thenReturn(0);
        } catch (UserApiException e) {
        ...
        Assert.assertEquals(MessagingConstants.RETURN_CODE_SUCCESS, actualRepsonse.getReplyStatus().getReturnCode().intValue());
        Assert.assertEquals(MessagingConstants.RETURN_STATUS_SUCCESS, actualRepsonse.getReplyStatus().getReturnStatus());
        Assert.assertEquals(UserMessageHandlerConst.MESSAGE_NOT_FOUND, actualRepsonse.getReplyStatus()
                .getMessage());
    }
    ...
    public void testError_API_Error() {
        String request = RMT2File.getFileContentsAsString("xml/admin/user/UserDeleteRequest.xml");
        try {
            when(this.mockApi.deleteUser(isA(Integer.class))).thenThrow(new UserApiException(API_ERROR));
        } catch (UserApiException e) {
        ...
        Assert.assertEquals(MessagingConstants.RETURN_STATUS_SUCCESS, actualRepsonse.getReplyStatus().getReturnStatus());
        Assert.assertEquals(UserMessageHandlerConst.MESSAGE_DELETE_ERROR, actualRepsonse.getReplyStatus()
                .getMessage());
    ...
    public void testValidation_Invalid_Transaction_Code() {
        String request = RMT2File
                .getFileContentsAsString("xml/admin/user/UserDeleteRequest_InvalidTransactionCode.xml");
    ...
    public void testValidation_Missing_Profile() {
        String request = RMT2File.getFileContentsAsString("xml/admin/user/UserDeleteRequest_MissingProfile.xml");
    ...
    @Test
    public void testValidation_Missing_Profile_User_Node() {
        String request = RMT2File
                .getFileContentsAsString("xml/admin/user/UserDeleteRequest_MissingUserSection.xml");
    ...
    public void testValidation_Too_Many_Records() {
        String request = RMT2File.getFileContentsAsString("xml/admin/user/UserDeleteRequest_TooManyRecords.xml");

...
import org.rmt2.api.handlers.admin.resource.ResourceQueryApiHandler;
...
public class ResourceQueryMessageHandlerTest extends BaseAuthenticationMessageHandlerTest {
     ...
     * 
     */
    public ResourceQueryMessageHandlerTest() {
        return;
    }
    ...
    public void testSuccess_Fetch() {
        String request = RMT2File.getFileContentsAsString("xml/admin/resource/ResourceQueryRequest.xml");

        try {
            when(this.mockApi.getResource(isA(ResourceDto.class))).thenReturn(SecurityMockDtoData.createUserResourceMockData());
        } catch (ResourceRegistryApiException e) {
            Assert.fail("Unable to setup mock stub for fetching resource type record");
        }
        
        MessageHandlerResults results = null;
        ResourceQueryApiHandler handler = new ResourceQueryApiHandler();
        try {
            results = handler.processMessage(ApiTransactionCodes.AUTH_RESOURCE_GET, request);
        ...
        Assert.assertEquals(5, actualRepsonse.getProfile().getResourcesInfo().getResource().size());
        Assert.assertEquals(5, actualRepsonse.getReplyStatus().getRecordCount().intValue());
        Assert.assertEquals(MessagingConstants.RETURN_CODE_SUCCESS, actualRepsonse.getReplyStatus().getReturnCode().intValue());
        Assert.assertEquals(MessagingConstants.RETURN_STATUS_SUCCESS, actualRepsonse.getReplyStatus().getReturnStatus());
        Assert.assertEquals(ResourceMessageHandlerConst.MESSAGE_FOUND, actualRepsonse.getReplyStatus().getMessage());
        ...
        for (int ndx = 0; ndx < actualRepsonse.getProfile().getResourcesInfo().getResource().size(); ndx++) {
            ...
            Assert.assertEquals(SecurityMockOrmDataFactory.TEST_RESOURCE_ID + ndx, a.getUid(), 0);
        }
    }
    ...
    public void testSuccess_Fetch_NotFound() {
        ...
        try {
            when(this.mockApi.getResource(isA(ResourceDto.class))).thenReturn(null);
        } catch (ResourceRegistryApiException e) {
            Assert.fail("Unable to setup mock stub for fetching resource type record");
        }
        ...
        Assert.assertEquals(MessagingConstants.RETURN_CODE_SUCCESS, actualRepsonse.getReplyStatus().getReturnCode().intValue());
        Assert.assertEquals(MessagingConstants.RETURN_STATUS_SUCCESS, actualRepsonse.getReplyStatus().getReturnStatus());
        Assert.assertEquals(ResourceMessageHandlerConst.MESSAGE_NOT_FOUND, actualRepsonse.getReplyStatus().getMessage());
    }
    ...
    public void testError_API_Error() {
        String request = RMT2File.getFileContentsAsString("xml/admin/resource/ResourceQueryRequest.xml");
        try {
            when(this.mockApi.getResource(isA(ResourceDto.class))).thenThrow(new ResourceRegistryApiException(API_ERROR));
        } catch (ResourceRegistryApiException e) {
            Assert.fail("Unable to setup mock stub for fetching resource type record");
        }
        ...
        Assert.assertEquals(MessagingConstants.RETURN_CODE_SUCCESS, actualRepsonse.getReplyStatus().getReturnCode().intValue());
        Assert.assertEquals(MessagingConstants.RETURN_STATUS_SUCCESS, actualRepsonse.getReplyStatus().getReturnStatus());
        Assert.assertEquals(ResourceMessageHandlerConst.MESSAGE_FETCH_ERROR, actualRepsonse.getReplyStatus().getMessage());
    }
    
    @Test
    public void testValidation_Invalid_Transaction_Code() {
        String request = RMT2File.getFileContentsAsString("xml/admin/resource/ResourceQueryRequest_InvalidTransactionCode.xml");
    ...
    public void testValidation_Missing_Criteria() {
        String request = RMT2File.getFileContentsAsString("xml/admin/resource/ResourceQueryRequest_MissingCriteria.xml");
    ...
    public void testValidation_Missing_Entity_Criteria() {
        String request = RMT2File.getFileContentsAsString("xml/admin/resource/ResourceQueryRequest_MissingResourceCriteria.xml");
        ...
        Assert.assertEquals(MessagingConstants.RETURN_CODE_FAILURE, actualRepsonse.getReplyStatus().getReturnCode().intValue());
        Assert.assertEquals(MessagingConstants.RETURN_STATUS_BAD_REQUEST, actualRepsonse.getReplyStatus().getReturnStatus());
        Assert.assertEquals(ResourceMessageHandlerConst.MESSAGE_MISSING_RESOURCE_CRITERIA_SECTION, actualRepsonse

...
@PrepareForTest({ AbstractDaoClientImpl.class, Rmt2OrmClientFactory.class, VideoMetadataBatchImportApiHandler.class,
        AvBatchFileFactory.class,
        SystemConfigurator.class })
public class VideoBatchImportMessageHandlerTest extends BaseMediaMessageHandlerTest {
     ...
     * 
     */
    public VideoBatchImportMessageHandlerTest() {
        return;
    }
    ...
    public void testSuccess() {
        String request = RMT2File.getFileContentsAsString("xml/batch/VideoMetadataImportBatchRequest.xml");
        try {
            when(this.mockApi.processBatch()).thenReturn(100);
        } catch (BatchFileException e) {
            Assert.fail("Unable to setup mock stub for video batch file import");
        }
        
        MessageHandlerResults results = null;
        VideoMetadataBatchImportApiHandler handler = new VideoMetadataBatchImportApiHandler();
        try {
            results = handler.processMessage(ApiTransactionCodes.MEDIA_VIDEO_METADATA_IMPORT_BATCH, request);

...
@PrepareForTest({ AbstractDaoClientImpl.class, Rmt2OrmClientFactory.class, MediaFileListenerStartApiHandler.class,
        DocumentContentApiFactory.class,
        SystemConfigurator.class })
public class MediaFileListenerStartMessageHandlerTest extends BaseMediaMessageHandlerTest {
     ...
     * 
     */
    public MediaFileListenerStartMessageHandlerTest() {
        return;
    }
    ...
    public void testSuccess_Operation() {
        String request = RMT2File.getFileContentsAsString("xml/listener/MediaFileListenerStartRequest.xml");

        doNothing().when(this.mockApi).startMediaFileListener();
        
        MessageHandlerResults results = null;
        MediaFileListenerStartApiHandler handler = new MediaFileListenerStartApiHandler();
        try {
            results = handler.processMessage(ApiTransactionCodes.MEDIA_FILE_LISTENER_START, request);
        ...
        Assert.assertEquals(MediaFileListenerApiHandlerConst.MESSAGE_STARTED, actualRepsonse.getReplyStatus().getMessage());
    }

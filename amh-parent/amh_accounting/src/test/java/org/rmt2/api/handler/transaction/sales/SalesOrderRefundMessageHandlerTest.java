...
/**
 * Tests the sales order refund API message handler
 * 
 * @author roy.terrell
 *
 */
@RunWith(PowerMockRunner.class)
@PrepareForTest({ AbstractDaoClientImpl.class, Rmt2OrmClientFactory.class, RefundSalesOrderApiHandler.class,
        SalesApiFactory.class,
        SystemConfigurator.class })
public class SalesOrderRefundMessageHandlerTest extends BaseAccountingMessageHandlerTest {
     ...
     * 
     */
    public SalesOrderRefundMessageHandlerTest() {
        return;
    }
    ...
    /**
     * Test the sales order refund method successfully
     */
    @Test
    public void testSuccess_Refund() {
        String request = RMT2File.getFileContentsAsString("xml/transaction/sales/SalesOrderRefundRequest.xml");

        try {
            when(this.mockApi.refundSalesOrder(isA(Integer.class))).thenReturn(1234567);
        } catch (SalesApiException e) {
            Assert.fail("Unable to setup mock stub for cancelling a sales order");
        }

        MessageHandlerResults results = null;
        RefundSalesOrderApiHandler handler = new RefundSalesOrderApiHandler();
        try {
            results = handler.processMessage(ApiTransactionCodes.ACCOUNTING_SALESORDER_REFUND, request);
        ...
        String expectedMsg = RMT2String.replace(SalesOrderHandlerConst.MSG_REFUND_SUCCESS,
    ...
    public void test_API_Error() {
        ...
        try {
            when(this.mockApi.refundSalesOrder(isA(Integer.class))).thenThrow(
        ...
        Assert.assertEquals(SalesOrderHandlerConst.MSG_REFUND_FAILURE, actualRepsonse.getReplyStatus().getMessage());
